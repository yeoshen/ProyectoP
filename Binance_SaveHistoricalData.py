...
def save_binance_historical_date(symbol, interval, start, end):
...
# client = Client("", "")
activo = "ETH"
balance = client.get_asset_balance(activo)
price = client.get_ticker(symbol=activo + 'USDT')
...
print("Tienes un saldo de {} {}, que representan {} dólares".format(
    balance['free'],
    activo,
    str(float(balance['free'])*float(price['lastPrice']))
))
